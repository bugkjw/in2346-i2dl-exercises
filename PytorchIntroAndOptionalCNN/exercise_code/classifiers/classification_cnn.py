...
class ClassificationCNN(nn.Module):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, kernel_size=7,
        ...
        h_pool = int(1 + (height - pool) / stride_pool)
        w_pool = int(1 + (width - pool) / stride_pool)
        # Initialize NN layers with trainable parameters.
        self.conv = nn.Conv2d(channels, num_filters, kernel_size, stride=stride_conv, padding=same)
        self.fc1 = nn.Linear(num_filters * h_pool * w_pool, hidden_dim)
        self.fc2 = nn.Linear(hidden_dim, num_classes)
        with torch.no_grad():
            self.conv.weight.mul_(weight_scale)
    ...
    def forward(self, x):
        ...
        x = F.max_pool2d(F.relu(self.conv(x)), kernel_size=pool, stride=stride_pool)
