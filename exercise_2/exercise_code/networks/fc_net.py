...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dout1, dW2, db2 = affine_backward(ds, cache2)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        if self.num_layers > 1:
            for ii in range(1, self.num_layers-1):
                self.params['W'+str(ii+1)] = np.random.randn(hidden_dims[ii-1], hidden_dims[ii])*weight_scale
                self.params['b'+str(ii+1)] = np.zeros(hidden_dims[ii])
        # Output layer
    ...
    def loss(self, X, y=None):
        ...
        cache = {}
        # Hidden layers
        out[1], cache[1] = affine_relu_forward(X, self.params['W1'], self.params['b1'])
        if self.num_layers > 1:
            for ii in range(1, self.num_layers-1):
                out[ii+1], cache[ii+1] = affine_relu_forward(out[ii],
                                                             self.params['W'+str(ii+1)],
                                                             self.params['b'+str(ii+1)])
        # Output layer
        scores, cache[self.num_layers] = affine_forward(out[self.num_layers-1],
        ...
        reg_loss = 0
        # Output layer
        reg_loss += np.sum(np.square(self.params['W'+str(self.num_layers)]))
        dout[self.num_layers-1], dW, db = affine_backward(dscores, cache[self.num_layers])
        grads['W'+str(self.num_layers)] = dW+self.reg*self.params['W'+str(self.num_layers)]
        grads['b'+str(self.num_layers)] = db
        # Hidden layers
        if self.num_layers > 1:
            for ii in range(self.num_layers-1, 1, -1):
                reg_loss += np.sum(np.square(self.params['W'+str(ii)]))
                dout[ii-1], dW, db = affine_relu_backward(dout[ii], cache[ii])
                grads['W'+str(ii)] = dW+self.reg*self.params['W'+str(ii)]
                grads['b'+str(ii)] = db
        reg_loss += np.sum(np.square(self.params['W1']))
        dx, dW, db = affine_relu_backward(dout[1], cache[1])
